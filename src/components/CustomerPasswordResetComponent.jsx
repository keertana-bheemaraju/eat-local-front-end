...
class CustomerPasswordResetComponent extends Component {
  constructor(props) {
    super(props);
  ...
  handleSubmit = (e) => {
    ...
    // clear error message
    this.setState({passwordResetMessage:""});
    ...
    if (formValid(this.state)) {
      ...
      const encrypted_password = UserServices.encryptWithSha512(pwd);
      const user = { username: email, password: encrypted_password };
      UserServices.resetCustomerPassword(user).then((res) => {
        const response = res.data;
        if (response.loginStatus == true) {
            toast("Password Reset Success. \n Please login.");
        } else {
            this.setState({passwordResetMessage:response.message})
        }
      });
    } else {
  ...
  render() {
    const { formErrors, passwordResetMessage } = this.state;
    return (
      <div className="wrapper">
        <div className="form-wrapper">
          ...
          <div className="standardButton">
            ...
            <Toaster/>
            {passwordResetMessage.length > 0 && (
              <span className="errorMessage">{passwordResetMessage}</span>
            )}
          </div>
        </div>
      </div>
